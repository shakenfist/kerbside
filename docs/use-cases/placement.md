...
- **The long hop carries an inspected session rather than a
  ...
  ([proxy-architecture.md](../proxy-architecture.md)). The
...
- **OpenStack.** This one does not work that way. Nova is
  ...
  that URL names — see
  [Kerbside for OpenStack](openstack.md). Per-office placement
...
([proxy-architecture.md](../proxy-architecture.md)). Offices
...
[documentation index](../index.md). Placement does not change
...
- [Multi-cloud aggregation](multi-cloud.md) — the inverse
