...
[configuration.md](configuration.md); this page explains why each one
exists.
...
[installation.md](installation.md#what-a-running-kerbside-needs)).
...
(see [spice-link-protocol.md](spice/spice-link-protocol.md)). A
...
walkthrough in [installation.md](installation.md) checks this with
`ss`).
...
The per-cloud pages in [use-cases/](index.md#use-cases) say which
hypervisor ports each cloud reports and the reachability traps
specific to it. See for example the "Network" sections of
[openstack.md](use-cases/openstack.md#network) and
[ovirt.md](use-cases/ovirt.md#network).
