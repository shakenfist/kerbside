...
Part of [PLAN-test-harness.md](PLAN-test-harness.md). The bulk of
...
- `docs/control-socket-protocol.md` — v1.0 spec. Scope
  ...
  envelope or verb".
- Phase 3 lives on the ryll `test-harness-control-socket`
  branch. The phase 3 PR is pending operator review; phase 6
  branches off that.
...
from the existing `test-harness-control-socket` branch.
...
- **ryll**: New branch `test-harness-phase-6` branched from
  the phase-3 branch `test-harness-control-socket`. Steps 6a,
  6b, 6c, and the ryll half of 6e all land there. Five
  commits total.
- **kerbside**: Existing `test-harness` branch. Step 6d and
  the kerbside half of 6e land there. Two commits total.
- The phase 3 ryll PR is independently mergeable; the phase 6
  ryll PR rebases onto `main` once phase 3 merges, or stays
  branched off `test-harness-control-socket` until then.
