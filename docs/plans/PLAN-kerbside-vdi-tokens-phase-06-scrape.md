...
| S2 | medium | sonnet | Where a node maintains its own `NodeAttributesData` at startup / on the monitor tick (find the caller of `_save_attributes` that writes `last_seen`/`installed_version` — the per-node daemon), read `/etc/pki/libvirt-spice/server-cert.pem` (guard: file may be absent on non-hypervisor nodes), parse the subject with `cryptography.x509.load_pem_x509_certificate(...).subject.rfc4514_string()` (match the exact string format the proxy's host-subject verifier compares — confirm against ryll's verifier / kerbside PR #114 semantics), and save it into `spice_server_cert_subject`. A missing/unreadable cert stores `''`, never crashes startup. Test with a fixture cert. |
